...
export default function BRDClarify({ analysis, questions, onGenerate, loading }) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Answer each question to give the AI the context it needs to generate your BRD.
        </p>
      </div>
        ...
        {questions.map((q, i) => (
          <div key={i} className="bg-white border border-gray-200 rounded-xl p-5">
            <div className="flex gap-3">
              <div className="w-6 h-6 rounded-full bg-violet-100 text-violet-700 flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-0.5">
                {i + 1}
              </div>
              <div className="flex-1 space-y-3">
                ...
                <textarea
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none"
                />
              </div>
            </div>
          </div>
