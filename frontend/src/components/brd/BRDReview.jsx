...
export default function BRDReview({ brd, onConfirm }) {
  const [fullBrd, setFullBrd] = useState(brd)
  const [sectionStates, setSectionStates] = useState({})

  const sections = parseSections(fullBrd)
  ...
  const handleRegenerate = async (heading) => {
    ...
    try {
      const res = await authFetch(`${API}/brd/regenerate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brd: fullBrd, section: heading, instruction: state.instruction }),
      })
            ...
            <div key={section.heading} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-violet-700 bg-white border border-violet-200 rounded-lg hover:bg-violet-50 transition-colors"
                >
              ...
              {state.open && (
                <div className="px-6 pb-5 pt-3 bg-violet-50/60 border-t border-violet-100 space-y-3">
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Make the outcomes more specific, add a success metric to the Success Criteria…"
                    rows={3}
                    className="w-full px-3 py-2.5 text-sm bg-white border border-violet-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent placeholder:text-gray-400 resize-none"
                  />
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium text-white bg-violet-600 rounded-lg hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      <SparklesIcon className="w-3.5 h-3.5" />
