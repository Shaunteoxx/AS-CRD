...
export default function CRDReview({ crd, onConfirm }) {
  ...
  const sections = parseSections(fullCrd)
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section.heading} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              {state.open && (
                <div className="px-6 pb-5 pt-3 bg-indigo-50/60 border-t border-indigo-100 space-y-3">
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Add a priority column to the requirements table, make the timeline more specific…"
                    rows={3}
                    className="w-full px-3 py-2.5 text-sm bg-white border border-indigo-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent placeholder:text-gray-400 resize-none"
                  />
