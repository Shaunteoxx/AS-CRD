...
export default function PRDUploadArea({ notes, setNotes, files, setFiles, onAnalyze, loading }) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="text-center mb-6">
        <h1 className="text-4xl font-bold text-gray-900" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>PRD Generator</h1>
        <p className="text-sm text-gray-500 mt-2">Outlining product features, user stories, and technical specifications.</p>
      </div>
      ...
      <div className="w-full bg-white rounded-2xl shadow-2xl overflow-hidden">
        <textarea
          ...
          rows={9}
          placeholder="Detail product features, user stories, or technical specifications…"
        ...
        {files.length > 0 && (
          <div className="flex flex-wrap gap-2 px-6 pb-3">
            {files.map(f => (
              <span
                key={f.name}
                className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1 bg-orange-50 border border-orange-200 text-orange-800 text-xs rounded-full max-w-[220px]"
              >
